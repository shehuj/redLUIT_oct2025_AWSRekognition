...
def main():
    ...
    print(f"[DEBUG] S3_BUCKET_PATH_PROD = {s3_bucket_path_prod}")
    print(f"[DEBUG] DYNAMODB_TABLE_PROD = {dynamodb_table}\n")
    
    # Validate required environment variables
    if not aws_region or not s3_bucket or not dynamodb_table:
        raise RuntimeError(
            "Missing one of AWS_REGION, S3_BUCKET, or DYNAMODB_TABLE_PROD. "
            "Got: aws_region=%s, "
            "s3_bucket=%s, "
            "dynamodb_table=%s, "
            "s3_bucket_path_prod=%s"
            % (aws_region, s3_bucket, dynamodb_table, s3_bucket_path_prod)
        )
    ...
    for image_file in os.listdir(image_dir):
        ...
        if image_file.lower().endswith((".jpg", ".png", ".jpeg")):
            ...
            try:
                ...
                print("Written to DynamoDB (prod).\n")
